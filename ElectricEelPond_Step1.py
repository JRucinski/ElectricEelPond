...
def main():
	...
	manage_files(args,comlist, names, read1files, read2files, receipt)
...
### Manage files ###
###--------------###
def manage_files(args, comlist, names, read1files, read2files, receipt):
	clean = True
	...
	for n in comlist:
		...
		if n in read1files or n in read2files:
	...
	for n in names:
		#if files have matching end read, i.e. are used in qsub
		if names[n] in read1files or names[n] in read2files:
			#check if already in output folder to avoid unnecessary duplication
